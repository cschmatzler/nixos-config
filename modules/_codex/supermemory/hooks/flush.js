...
// src/services/transcript.ts
var SESSIONS_DIR = (0, import_node_path4.join)((0, import_node_os5.homedir)(), ".codex", "sessions");
function findTranscriptPath(sessionId) {
  if (!(0, import_node_fs4.existsSync)(SESSIONS_DIR)) {
    return null;
  }
  ...
  const todayDir = (0, import_node_path4.join)(SESSIONS_DIR, year, month, day);
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    const pastDir = (0, import_node_path4.join)(SESSIONS_DIR, y, m, d);
  ...
  try {
    ...
    for (const file of files) {
      if (file.endsWith(".jsonl") && file.includes(sessionId)) {
        return (0, import_node_path4.join)(dir, file);
      }
    }
  } catch {
  }
...
function parseTranscript(transcriptPath) {
  ...
  try {
    const raw = (0, import_node_fs4.readFileSync)(transcriptPath, "utf-8");
...
var TRACKER_DIR = (0, import_node_path5.join)((0, import_node_os6.homedir)(), ".codex-supermemory", "trackers");
function ensureTrackerDir() {
  if (!(0, import_node_fs5.existsSync)(TRACKER_DIR)) {
    (0, import_node_fs5.mkdirSync)(TRACKER_DIR, { recursive: true });
  }
}
function getLastCapturedIndex(sessionId) {
  ensureTrackerDir();
  const trackerFile = (0, import_node_path5.join)(TRACKER_DIR, `${sessionId}.txt`);
  if ((0, import_node_fs5.existsSync)(trackerFile)) {
    const content = (0, import_node_fs5.readFileSync)(trackerFile, "utf-8").trim();
...
function setLastCapturedIndex(sessionId, index) {
  ensureTrackerDir();
  const trackerFile = (0, import_node_path5.join)(TRACKER_DIR, `${sessionId}.txt`);
  (0, import_node_fs5.writeFileSync)(trackerFile, String(index));
}
...
async function captureEntries(caller, client, sessionId, transcriptPath, tags, options = {}) {
  ...
  if (!transcriptPath || !(0, import_node_fs6.existsSync)(transcriptPath)) {
...
async function main() {
  let rawInput = "";
  try {
    rawInput = (0, import_node_fs7.readFileSync)(0, "utf-8");
  } catch {
    return;
  }
  if (!isConfigured()) {
