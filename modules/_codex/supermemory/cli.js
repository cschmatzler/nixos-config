...
var CODEX_DIR = (0, import_node_path2.join)((0, import_node_os2.homedir)(), ".codex");
var CODEX_CONFIG_TOML = (0, import_node_path2.join)(CODEX_DIR, "config.toml");
var CODEX_HOOKS_JSON = (0, import_node_path2.join)(CODEX_DIR, "hooks.json");
var SUPERMEMORY_HOOKS_DIR = (0, import_node_path2.join)(CODEX_DIR, "supermemory");
var RECALL_SCRIPT = (0, import_node_path2.join)(SUPERMEMORY_HOOKS_DIR, "recall.js");
var FLUSH_SCRIPT = (0, import_node_path2.join)(SUPERMEMORY_HOOKS_DIR, "flush.js");
var CODEX_SKILLS_DIR = (0, import_node_path2.join)((0, import_node_os2.homedir)(), ".codex", "skills");
var RECALL_TIMEOUT_SECONDS = 90;
var FLUSH_TIMEOUT_SECONDS = 60;
var SKILLS = [
...
var DIST_HOOKS_DIR = (0, import_node_path2.join)(SCRIPT_DIR, "hooks");
function ensureCodexDir() {
  (0, import_node_fs2.mkdirSync)(CODEX_DIR, { recursive: true });
  (0, import_node_fs2.mkdirSync)(SUPERMEMORY_HOOKS_DIR, { recursive: true });
}
function mergeConfigToml(enable) {
  if (!enable && !(0, import_node_fs2.existsSync)(CODEX_CONFIG_TOML)) {
    return;
  }
  let config = {};
  if ((0, import_node_fs2.existsSync)(CODEX_CONFIG_TOML)) {
    try {
      const content = (0, import_node_fs2.readFileSync)(CODEX_CONFIG_TOML, "utf-8");
  ...
  (0, import_node_fs2.writeFileSync)(CODEX_CONFIG_TOML, TOML.stringify(config));
}
function normalizeHookEvents(raw) {
  if (!raw || typeof raw !== "object") return {};
  const maybeWrapped = raw;
  const events = maybeWrapped.hooks && typeof maybeWrapped.hooks === "object" ? maybeWrapped.hooks : maybeWrapped;
...
function ensureHookRegistered(groups, command2, timeout, statusMessage) {
  const exists = groups.some((g) => g.hooks.some((h) => h.command === command2));
  if (exists) {
    for (const group of groups) {
      for (const hook of group.hooks) {
        if (hook.command === command2) {
    ...
    const entry = { type: "command", command: command2, timeout, statusMessage };
    if (globalGroup) {
...
function removeHookCommands(groups, commands) {
  return groups.map((g) => ({ ...g, hooks: g.hooks.filter((h) => !commands.includes(h.command)) })).filter((g) => g.hooks.length > 0);
}
function mergeHooksJson(add) {
  if (!add && !(0, import_node_fs2.existsSync)(CODEX_HOOKS_JSON)) {
    return;
  }
  let hooks = {};
  if ((0, import_node_fs2.existsSync)(CODEX_HOOKS_JSON)) {
    try {
      const content = (0, import_node_fs2.readFileSync)(CODEX_HOOKS_JSON, "utf-8");
  ...
  if (add) {
    ...
    const oldCaptureCmd = `node ${(0, import_node_path2.join)(SUPERMEMORY_HOOKS_DIR, "capture.js")}`;
  ...
  (0, import_node_fs2.writeFileSync)(CODEX_HOOKS_JSON, JSON.stringify({ hooks }, null, 2));
}
function install() {
  console.log("Installing codex-supermemory...\n");
  ensureCodexDir();
  const recallSrc = (0, import_node_path2.join)(DIST_HOOKS_DIR, "recall.js");
  const flushSrc = (0, import_node_path2.join)(DIST_HOOKS_DIR, "flush.js");
  if (!(0, import_node_fs2.existsSync)(recallSrc) || !(0, import_node_fs2.existsSync)(flushSrc)) {
    console.error("Error: Hook scripts not found. Please reinstall the package.");
    process.exit(1);
  }
  (0, import_node_fs2.copyFileSync)(recallSrc, RECALL_SCRIPT);
  (0, import_node_fs2.copyFileSync)(flushSrc, FLUSH_SCRIPT);
  const oldCapture = (0, import_node_path2.join)(SUPERMEMORY_HOOKS_DIR, "capture.js");
  if ((0, import_node_fs2.existsSync)(oldCapture)) {
    (0, import_node_fs2.rmSync)(oldCapture);
  }
  for (const { name, script } of SKILLS) {
    (0, import_node_fs2.copyFileSync)(
      (0, import_node_path2.join)(SCRIPT_DIR, "skills", script),
      (0, import_node_path2.join)(SUPERMEMORY_HOOKS_DIR, script)
    );
    const skillDir = (0, import_node_path2.join)(CODEX_SKILLS_DIR, name);
    (0, import_node_fs2.mkdirSync)(skillDir, { recursive: true });
    (0, import_node_fs2.copyFileSync)(
      (0, import_node_path2.join)(SCRIPT_DIR, "skills", name, "SKILL.md"),
      (0, import_node_path2.join)(skillDir, "SKILL.md")
    );
  }
...
function uninstall() {
  ...
  if ((0, import_node_fs2.existsSync)(SUPERMEMORY_HOOKS_DIR)) {
    (0, import_node_fs2.rmSync)(SUPERMEMORY_HOOKS_DIR, { recursive: true, force: true });
    console.log(`\u2713 Removed ${SUPERMEMORY_HOOKS_DIR}`);
  }
  for (const { name } of SKILLS) {
    const skillDir = (0, import_node_path2.join)(CODEX_SKILLS_DIR, name);
    if ((0, import_node_fs2.existsSync)(skillDir)) {
      (0, import_node_fs2.rmSync)(skillDir, { recursive: true, force: true });
    }
  }
...
function status() {
  ...
  const credentialsApiKey = !envApiKey ? loadCredentials() : void 0;
  const apiKey = envApiKey || credentialsApiKey;
  const apiKeySource = envApiKey ? "SUPERMEMORY_CODEX_API_KEY env var" : credentialsApiKey ? "credentials file (~/.codex/supermemory/credentials.json)" : null;
  const hooksInstalled = (0, import_node_fs2.existsSync)(RECALL_SCRIPT) && (0, import_node_fs2.existsSync)(FLUSH_SCRIPT);
  const hooksJsonExists = (0, import_node_fs2.existsSync)(CODEX_HOOKS_JSON);
  const configTomlExists = (0, import_node_fs2.existsSync)(CODEX_CONFIG_TOML);
  let hooksEnabled = false;
  if (hooksJsonExists) {
    try {
      const hooks = normalizeHookEvents(JSON.parse((0, import_node_fs2.readFileSync)(CODEX_HOOKS_JSON, "utf-8")));
      const recallCmd = `node ${RECALL_SCRIPT}`;
      const flushCmd = `node ${FLUSH_SCRIPT}`;
      const recallRegistered = hooks.UserPromptSubmit?.some(
        (g) => g.hooks.some((h) => h.command === recallCmd)
      );
      const flushRegistered = hooks.Stop?.some(
        (g) => g.hooks.some((h) => h.command === flushCmd)
      );
  ...
  const skillsInstalled = SKILLS.every(
    ({ name }) => (0, import_node_fs2.existsSync)((0, import_node_path2.join)(CODEX_SKILLS_DIR, name, "SKILL.md"))
  );
