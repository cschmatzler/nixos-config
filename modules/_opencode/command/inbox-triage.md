...
5. For each single envelope ID, do all checks before any move/delete:
    ...
    - If needed for classification, download attachments: `himalaya attachment download -f "<source>" <id> --dir /tmp/himalaya-triage`.
    - Always `rm` downloaded files from `/tmp/himalaya-triage` after processing.
