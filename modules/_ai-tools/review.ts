...
const LOCAL_CHANGES_REVIEW_INSTRUCTIONS =
	"Also include local working-tree changes (staged, unstaged, and untracked files) from this branch. Use `jj status` and `jj diff -r @` so local fixes are part of this review cycle.";

const BASE_BRANCH_PROMPT_WITH_MERGE_BASE =
	"Review the code changes against the base branch '{baseBranch}'. The merge base commit for this comparison is {mergeBaseSha}. Run `jj diff --from {mergeBaseSha}` to inspect the changes relative to {baseBranch}. Provide prioritized, actionable findings.";

const BASE_BRANCH_PROMPT_FALLBACK =
	"Review the code changes against the base branch '{branch}'. Start by finding the merge diff between the current branch and {branch}'s upstream e.g. (`jj log -r 'fork_point(@ | {branch})' --no-graph -T 'commit_id.short() ++ \"\\n\"'`), then run `jj diff --from <that-sha>` to see what changes we would merge into the {branch} branch. Provide prioritized, actionable findings.";
...
const PULL_REQUEST_PROMPT =
	'Review pull request #{prNumber} ("{title}") against the base branch \'{baseBranch}\'. The merge base commit for this comparison is {mergeBaseSha}. Run `jj diff --from {mergeBaseSha}` to inspect the changes that would be merged. Provide prioritized, actionable findings.';

const PULL_REQUEST_PROMPT_FALLBACK =
	'Review pull request #{prNumber} ("{title}") against the base branch \'{baseBranch}\'. Start by finding the merge base between the current branch and {baseBranch} (e.g., `jj log -r \'fork_point(@ | {baseBranch})\' --no-graph -T \'commit_id.short() ++ "\\n"\'`), then run `jj diff --from <that-sha>` to see the changes that would be merged. Provide prioritized, actionable findings.';
...
/**
 * Get the merge base between the working copy and a bookmark
 */
async function getMergeBase(
	...
	try {
		const { stdout: mergeBase, code } = await pi.exec("jj", [
			"log",
			"--no-graph",
			"-r",
			`fork_point(@ | ${branch})`,
...
/**
 * Checkout a PR using Jujutsu
 */
async function checkoutPr(
	pi: ExtensionAPI,
	_headPrNumber: number,
	headBranch: string,
): Promise<{ success: boolean; error?: string }> {
	const fetchResult = await pi.exec("jj", ["git", "fetch", "--branch", headBranch]);
	if (fetchResult.code !== 0) {
		return { success: false, error: fetchResult.stderr || fetchResult.stdout || "Failed to fetch PR branch" };
	}

	let checkoutResult = await pi.exec("jj", ["new", headBranch]);
	if (checkoutResult.code !== 0) {
		checkoutResult = await pi.exec("jj", ["new", `${headBranch}@origin`]);
	}

	if (checkoutResult.code !== 0) {
		return {
			success: false,
			error: checkoutResult.stderr || checkoutResult.stdout || "Failed to checkout PR branch",
		};
	}
...
async function getDefaultBranch(pi: ExtensionAPI): Promise<string> {
	const { stdout, code } = await pi.exec("jj", ["bookmark", "list", "-r", "trunk()", "-T", 'name ++ "\\n"']);
	if (code === 0 && stdout.trim()) {
		return stdout.trim().split("\n")[0];
	}
...
export default function reviewExtension(pi: ExtensionAPI) {
	...
	async function showPrInput(ctx: ExtensionContext): Promise<ReviewTarget | null> {
		...
		const checkoutResult = await checkoutPr(pi, prNumber, prInfo.headBranch);
