...
- The profiles need writable agent state (`~/.pi` or Claude state), Plannotator state, opensrc state, npm cache for existing runtime `npx` flows, and the current worktree. The profiles set `TMPDIR` to `~/.pi/tmp` or `~/.claude/tmp`, and Home Manager provisions those directories, keeping Jiti and other read/write temporary artifacts inside already-granted agent state. The inherited Linux system group still grants write-only access to shared `/tmp`; it does not grant reads. SSH, cloud credentials, browser data, shell configuration/history, Docker, and other unrelated home paths remain outside the grants.
...
- Filesystem denial rules alone do not block Linux `connect(2)` to pathname Unix sockets. Set `linux.af_unix_mediation = "pathname"` so pathname sockets are default-deny unless granted through `filesystem.unix_socket*`: [Landlock pathname socket mediation](https://github.com/nolabs-ai/nono/blob/v0.73.0/docs/cli/internals/landlock.mdx#pathname-unix-socket-mediation). The profiles grant connect-only access to `/run/nscd/socket` for NixOS NSS lookups and bind/connect access only below each agent's private `TMPDIR` for the configured Nvim external editor and other private temporary IPC. Home Manager creates the stable parent before nono resolves the profile; random per-launch socket paths are not granted individually.
- Do not grant the Nix daemon, Docker, Herdr, D-Bus, or SSH-agent sockets by default. Any one delegates work to a host service and weakens containment. Devenv and Nix evaluation/build operations require the Nix daemon on this multi-user Nix installation, so they intentionally do not work in the daily Pi profile. Run the underlying package explicitly outside the wrapper when a session must operate Nix; Docker, Herdr, D-Bus, and SSH agent remain denied in normal sessions.
...
Listen-only ports are explicit: Pi's OpenAI Codex browser OAuth callback (1455) and Plannotator (20000). Port 1455 is only needed during browser-based `/login openai-codex`; Pi also supports device-code or manual callback entry. Plannotator binds port 20000 because `PLANNOTATOR_PORT` is fixed to that value.
...
The global Git configuration invokes `gh auth git-credential`, and agents need authenticated GitHub access to push branches and create pull requests. The profiles therefore grant read-only access to the exact `~/.config/gh/config.yml` and `~/.config/gh/hosts.yml` files, not read/write access to the whole directory. This exposes the GitHub token to the agent and every loaded in-process extension; use a narrowly scoped GitHub token and treat those extensions as one credential domain. A proxy route can hide the token for GitHub API calls, but does not by itself broker ordinary authenticated Git-over-HTTPS pushes.
...
10. Confirm that `pi` is the only sandbox launcher and directly execs nono with the Pi profile and package.
